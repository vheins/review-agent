...
export class OrchestrationSetService {
  ...
  async createSet(
    name: string,
    description: string,
    prs: { prId: number; dependencyPrId?: number }[] = [],
  ): Promise<string> {
    const setId = uuidv4();
  ...
  async getSetStatus(setId: string): Promise<any> {
    ...
    const completed = set.members.filter(
      m => m.pullRequest.status === 'merged' || m.pullRequest.status === 'closed',
    ).length;
  ...
  async assignReviewersToSet(setId: string, reviewerIds: number[]): Promise<void> {
    ...
    this.logger.log(
      `Assigning ${reviewerIds.length} reviewers to set ${setId} (${members.length} PRs)`,
    );
