...
export class MissionControlService implements OnModuleInit {
  ...
  async recoverSessions(): Promise<void> {
    ...
    for (const session of activeSessions) {
      await this.ledger?.append(
        ...
        'recovery',
        'system',
        `System restart recovery. Original status: ${session.status}`,
      );
  ...
  async startMission(
    ...
    const repository =
      typeof pr.repository === 'string'
        ? pr.repository
        : pr.repository?.nameWithOwner || pr.repository;
    ...
    await this.ledger?.append(
      ...
      'mission_start',
      'system',
      `Started mission session`,
      { prTitle: pr.title },
    );
    this.gateway?.broadcastMissionUpdate(savedSession);
    await this.audit?.logAction('mission_start', 'system', 'mission_session', savedSession.id, {
      runbookType,
      mode,
    });

    const firstStepDef = RUNBOOKS[runbookType][0];
    await this.createStep(savedSession.id, firstStepDef.role, firstStepDef.name, {
      action: firstStepDef.action,
    });
    return savedSession;
  }
  ...
  async createStep(
    ...
    if (action) {
      if (
        session.operationMode === 'observe' &&
        (action === 'fix' || action === 'merge' || action === 'review')
      ) {
        return this.createSkippedStep(
          sessionId,
          role,
          stepName,
          'Blocked by observe mode',
          metadata,
        );
      }
      ...
      if (session.operationMode === 'fix-safe' && action === 'merge') {
        return this.createSkippedStep(
          ...
          role,
          stepName,
          'Blocked by fix-safe mode',
          metadata,
        );
      }

      if (this.confidenceScorer && (action === 'fix' || action === 'merge')) {
        const shouldGate = await this.confidenceScorer.shouldGateAction(
          session.prNumber,
          session.repository,
          action,
        );
        if (shouldGate) {
          ...
          await this.ledger?.append(
            ...
            'confidence_gate',
            'system',
            `Action ${action} gated due to low confidence`,
          );
          const step = await this.createGatedStep(
            sessionId,
            role,
            stepName,
            `Gated due to low confidence for ${action}`,
            metadata,
          );
          if (this.humanOverride)
            await this.humanOverride.requestOverride(
              sessionId,
              `Confidence threshold breach for ${action}`,
              { action },
            );
    ...
    await this.ledger?.append(sessionId, 'step_start', role, `Started step: ${stepName}`, {
      stepId: savedStep.id,
    });
    await this.sessionRepository.update(sessionId, {
      currentRole: role,
  ...
  private async createSkippedStep(
    sessionId: string,
    role: string,
    stepName: string,
    reason: string,
    ...
    await this.ledger?.append(
      ...
      'step_skip',
      role,
      `Skipped step: ${stepName}. Reason: ${reason}`,
    );
    setTimeout(() => this.advanceMission(sessionId), 100);
    return savedStep;
  }

  private async createGatedStep(
    sessionId: string,
    role: string,
    stepName: string,
    reason: string,
    ...
    const step = this.stepRepository.create({
      ...
      status: 'awaiting_human',
      startedAt: new Date(),
      metadata: { ...metadata, gateReason: reason },
    });
  ...
  async advanceMission(sessionId: string): Promise<void> {
    ...
    const completedStepsCount = session.steps.filter(
    ...
    await this.createStep(sessionId, nextStepDef.role, nextStepDef.name, {
      action: nextStepDef.action,
    });
  }
  ...
  async completeStep(
    ...
    if (status === 'completed') await this.advanceMission(step.sessionId);
      ...
      await this.sessionRepository.update(step.sessionId, {
        status: 'failed',
        failureReason: `Step ${step.stepName} failed`,
        updatedAt: new Date(),
      });
  ...
  async pauseMission(sessionId: string, reason: string): Promise<void> {
    ...
    const updatedSession = await this.sessionRepository.findOne({ where: { id: sessionId } });
    this.gateway?.broadcastMissionUpdate(updatedSession);
    await this.audit?.logAction('mission_pause', 'system', 'mission_session', sessionId, {
      reason,
    });
  }
