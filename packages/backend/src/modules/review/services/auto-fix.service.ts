...
export class AutoFixService {
  ...
  async runProjectFixers(repoDir: string) {
    const hasEslint =
      (await fs.pathExists(path.join(repoDir, '.eslintrc.json'))) ||
      (await fs.pathExists(path.join(repoDir, 'package.json')));

    if (hasEslint) {
      try {
        this.logger.log('Running eslint --fix...');
        await this.github.execaVerbose('yarn', ['lint', '--fix'], {
          cwd: repoDir,
          allowFail: true,
        });
      } catch (e) {}
    ...
    if (hasPrettier) {
      try {
        ...
        await this.github.execaVerbose('npx', ['prettier', '--write', '.'], {
          cwd: repoDir,
          allowFail: true,
        });
      } catch (e) {}
  ...
  async fixConflicts(
    repoDir: string,
    baseBranch: string,
    prInfo?: { number: number; title: string; repository: string; headRefName: string },
  ): Promise<boolean> {
    try {
      ...
      if (exitCode !== 0) {
        ...
        if (conflictedFiles.length === 0) {
          ...
          await this.github.execaVerbose('git', ['merge', '--abort'], {
            cwd: repoDir,
            allowFail: true,
          });
          return false;
        ...
        for (const relativePath of conflictedFiles) {
          ...
          const resolvedContent = await this.aiFixGen.resolveConflicts(
            content,
            relativePath,
            repoDir,
            prInfo,
        ...
        await this.github.execaVerbose(
          'git',
          ['commit', '-m', `fix(conflict): selesaikan konflik di ${conflictFilesStr}`],
          { cwd: repoDir },
        );
        ...
        await this.github.execaVerbose('git', ['push', 'origin', 'HEAD', '--force-with-lease'], {
          cwd: repoDir,
        });
        return true;
      }
