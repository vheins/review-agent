...
export function ConfigTab() {
  const {
    snapshot, repositoryId, refreshRepositoryConfig,
    configForm, setConfigForm, configValidation, handleSaveConfig,
    ruleForm, setRuleForm, handleTestRule, handleSaveRule, ruleFeedback, setRuleFeedback,
    configData,
  } = useDashboard();
          ...
          <form className="grid gap-4" onSubmit={handleSaveConfig}>
            ...
            <Card className="p-4">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="grid gap-2"><span className="text-sm font-medium text-muted-foreground">Review Interval (sec)</span><Input type="number" value={configForm.reviewInterval} onChange={(event) => setConfigForm((prev) => ({ ...prev, reviewInterval: event.target.value }))} /></div>
                <div className="grid gap-2"><span className="text-sm font-medium text-muted-foreground">Review Mode</span><Select value={configForm.reviewMode} onChange={(event) => setConfigForm((prev) => ({ ...prev, reviewMode: event.target.value }))}><option value="comment">Comment</option><option value="fix">Fix</option></Select></div>
                <div className="grid gap-2"><span className="text-sm font-medium text-muted-foreground">AI Executor</span><Select value={configForm.aiExecutor} onChange={(event) => setConfigForm((prev) => ({ ...prev, aiExecutor: event.target.value }))}><option value="gemini">Gemini</option><option value="copilot">Copilot</option><option value="kiro">Kiro</option><option value="claude">Claude</option><option value="codex">Codex</option><option value="opencode">OpenCode</option></Select></div>
                <div className="grid gap-2"><span className="text-sm font-medium text-muted-foreground">Auto Merge</span><Select value={configForm.autoMerge} onChange={(event) => setConfigForm((prev) => ({ ...prev, autoMerge: event.target.value }))}><option value="true">Enabled</option><option value="false">Disabled</option></Select></div>
              </div>
            </Card>
            <Card className="p-4">
              <div className="mb-4">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="grid gap-2"><span className="text-sm font-medium text-muted-foreground">Severity Threshold</span><Input type="number" value={configForm.severityThreshold} onChange={(event) => setConfigForm((prev) => ({ ...prev, severityThreshold: event.target.value }))} /></div>
                <div className="grid gap-2"><span className="text-sm font-medium text-muted-foreground">Auto Merge Health Threshold</span><Input type="number" value={configForm.autoMergeHealthThreshold} onChange={(event) => setConfigForm((prev) => ({ ...prev, autoMergeHealthThreshold: event.target.value }))} /></div>
              </div>
              <div className="mt-4 grid gap-2"><span className="text-sm font-medium text-muted-foreground">Required Checks</span><Input value={configForm.requiredChecks} onChange={(event) => setConfigForm((prev) => ({ ...prev, requiredChecks: event.target.value }))} /></div>
            </Card>
            <Card className="p-4">
              <div className="mb-4">
                <div className="flex items-center gap-2 text-sm font-medium">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="grid gap-2">
                  ...
                  <Select value={configForm.logLevel} onChange={(event) => setConfigForm((prev) => ({ ...prev, logLevel: event.target.value }))}>
        ...
        <Section eyebrow="Rules" title="Custom Rule Editor" description="Create team-specific static checks and validate them against sample code before saving.">
          <form className="grid gap-4" onSubmit={handleSaveRule}>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <div className="grid gap-2"><span className="text-sm font-medium text-muted-foreground">Rule Name</span><Input value={ruleForm.rule_name} onChange={(event) => setRuleForm((prev) => ({ ...prev, rule_name: event.target.value }))} /></div>
              <div className="grid gap-2"><span className="text-sm font-medium text-muted-foreground">Type</span><Select value={ruleForm.rule_type} onChange={(event) => setRuleForm((prev) => ({ ...prev, rule_type: event.target.value }))}><option value="regex">Regex</option><option value="ast">AST</option></Select></div>
              <div className="grid gap-2"><span className="text-sm font-medium text-muted-foreground">Severity</span><Select value={ruleForm.severity} onChange={(event) => setRuleForm((prev) => ({ ...prev, severity: event.target.value }))}><option value="critical">Critical</option><option value="high">High</option><option value="warning">Warning</option><option value="info">Info</option></Select></div>
            </div>
            <div className="grid gap-2"><span className="text-sm font-medium text-muted-foreground">Pattern</span><Textarea rows={3} value={ruleForm.pattern} onChange={(event) => setRuleForm((prev) => ({ ...prev, pattern: event.target.value }))} /></div>
            <div className="grid gap-2"><span className="text-sm font-medium text-muted-foreground">Message</span><Textarea rows={2} value={ruleForm.message} onChange={(event) => setRuleForm((prev) => ({ ...prev, message: event.target.value }))} /></div>
            <div className="grid gap-2"><span className="text-sm font-medium text-muted-foreground">Sample Code For Validation</span><Textarea id="ruleSampleCode" rows={5} value={ruleForm.sampleCode} onChange={(event) => setRuleForm((prev) => ({ ...prev, sampleCode: event.target.value }))} /></div>
            <div className="flex flex-wrap gap-3">
